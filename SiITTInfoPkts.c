...
//------------------------------------------------------------------------------
void InfoACPInterruptHandler( void ){          // SiI_RX_NewInfo_ACP
BYTE abInfoPkt[2];
...
// call ACP processing
        if( abInfoPkt[1] > 1 ){
            SiI_Inf.bGlobStatus |= SiI_RX_GlobalHDMI_ACP; // Audio is protected
        }
        else {
             // Flag repported on upper layer, Enable Digital Output
...
//                       to be able receive all types of Info, call checking ACP status
// Accepts: none
// Returns: none
// Globals: none
//------------------------------------------------------------------------------
