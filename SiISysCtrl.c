...
// Function Description:  This function is used for time measuring, time differnce shouldn't
//                        exceed 0xFFFF ticks
// Accepts: none
...
// Function Description: this function takes time for the last executed API and convert it
//                       from ticks to ms
// Accepts: none
...
BYTE bResult = 0;
static BYTE sm_bVideo = SiI_RX_VS_PowerDown;          // values should be same as initalized  in siiInitilizeSystemData
...
// Function Description:  This function is used to Re-inialize HDMI RX after Hardware reset
// Accepts: none
// Returns: none
// Globals: none
//------------------------------------------------------------------------------
...
AudioOutputFormatType AudioOutputFormat;
    ...
    siiGetRXInitParameters ( abData );             // Gets MClock divider and active TMDS core
    siiGetAudioOutputFormat( &AudioOutputFormat ); // Get Audio Format Setting, some setting stored in RX regs. so it's
...
// Now it's need to restore Aideo Output Format
    siiSetAudioOutputFormat( &AudioOutputFormat ); // Restore RX Audio Format Configuration

// Set Video Format static settings
    abData[0] = abData[1] = abData[2] = 0;         // Restore RX Video Format Configuration
    siiGetVideoFormatData( abData );       //YMA restore the configuration, or outpout depth losted		
    ...
    siiSetVideoFormatData( abData ); // Update RX Video format cfg. at Vid. Mode/Ctrl regs.
