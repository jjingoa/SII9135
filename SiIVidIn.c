...
BYTE bECode = FALSE;
    ...
    siiIIC_RX_RWBitsInByteP0(RX_SYS_SW_SWTCHC_ADDR, ( RX_BIT_RX0_EN | RX_BIT_DDC0_EN | RX_BIT_RX1_EN | RX_BIT_DDC1_EN )
                                                      , CLR );

    if ( bVideoInputChannels == SiI_RX_VInCh1 )
        siiIIC_RX_RWBitsInByteP0( RX_SYS_SW_SWTCHC_ADDR, RX_BIT_RX0_EN | RX_BIT_DDC0_EN, SET );
    else
        siiIIC_RX_RWBitsInByteP0( RX_SYS_SW_SWTCHC_ADDR, RX_BIT_RX1_EN | RX_BIT_DDC1_EN, SET);
...
void siiChangeVideoInput( BYTE bVideoInputChannel ){
    ...
    if ( ( SiI_Ctrl.bDevId ==  RX_SiI9011 ) && ( bVideoInputChannel != SiI_RX_VInCh1) ) {
        ...
        //SiI_Ctrl.bVidInChannel = bVideoInputChannel; //YMA set already
        if ( siiIsVideoOn() && ( SiI_Inf.bGlobStatus & SiI_RX_GlobalHDMI_Detected ) ) {
            siiSetSM_ReqVidInChange();  // Gives time for Audio soft muting
