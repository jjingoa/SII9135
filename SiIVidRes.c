...
// Function Description:  checks Sync Detect ( asserted when cirtain number of DE counted )
//------------------------------------------------------------------------------
...
//
    if ( ( SiI_Ctrl.bDevId == RX_SiI9133 ) ||( SiI_Ctrl.bDevId == RX_SiI9135 || SiI_Ctrl.bDevId ==  RX_SiI9125) || ( SiI_Ctrl.bDevId == RX_SiIIP11 ) ) {
        for (  i = 0; i < 8; i++ ) {
            wXClks += GetXClockW(); // by doing this we will get average XClk
        }
        SiI_Inf.Sync.bFPix = (BYTE)( (WORD) ( SII_XCLOCK_OSC_SCALED2047_FOR_CALK_FPIX / wXClks ) );
    }
    else {
        for (  i = 0; i < 8; i++ ) {
            wXClks += GetXClock(); // by doing this we will get average XClk
        }
        SiI_Inf.Sync.bFPix = (BYTE)( (WORD) ( SII_XCLOCK_OSC_SCALED_FOR_CALK_FPIX / wXClks ) );
    }
    ...
    if ( ( SiI_Ctrl.bDevId == RX_SiI9133 ) ||( SiI_Ctrl.bDevId == RX_SiI9135|| SiI_Ctrl.bDevId ==  RX_SiI9125) || ( SiI_Ctrl.bDevId == RX_SiIIP11 ) ) {
        ...
        wVFreq = (WORD)( SII_XCLOCK_OSC_SCALED2047_AND_MUL100 / dwXclksPerFrame );
        }
    else {
	    dwXclksPerFrame *= (DWORD)wXClks;
        dwXclksPerFrame >>= 10; // div ( 8 * 128 )
        wVFreq = (WORD)( SII_XCLOCK_OSC_SCALED_AND_MUL100 / dwXclksPerFrame );
        }
...
BOOL qResult = FALSE;
    ...
    if ( VModeTables[Index].Tag.RefrTypeVHPol == SyncInform->RefrTypeVHPol )
        if ( abs_diff(VModeTables[Index].Tag.VFreq, SyncInform->VFreq ) < V_TOLERANCE )
            if ( abs_diff(VModeTables[Index].Tag.Total.Lines, SyncInform->HTot) < LINES_TOLERANCE )
                if ( abs_diff(VModeTables[Index].Tag.Total.Pixels, SyncInform->ClocksPerLine) < PIXELS_TOLERANCE )
                    qResult = TRUE;
    return qResult;
...
// Function Description:  Compare timing inform with an entry of video
//                        mode reference table, but polarity information is ignored
//------------------------------------------------------------------------------
static BYTE CompareWithVMTableRefIgnorPol( BYTE Index, SyncInfoType * SyncInform){
BOOL qResult = FALSE;

    if ( (VModeTables[Index].Tag.RefrTypeVHPol & INTERLACE_MASK )==(SyncInform->RefrTypeVHPol & INTERLACE_MASK) )
        if ( abs_diff( VModeTables[Index].Tag.VFreq, SyncInform->VFreq ) <  V_TOLERANCE )
            if ( abs_diff( VModeTables[Index].Tag.Total.Lines, SyncInform->HTot) < LINES_TOLERANCE )
                if ( abs_diff( VModeTables[Index].Tag.Total.Pixels, SyncInform->ClocksPerLine) < PIXELS_TOLERANCE )
                    qResult = TRUE;
...
// Function Description: This function used for search video resolution modes through
//                       reference tables
//------------------------------------------------------------------------------
...
// Function Description: This function is used to detect of sync data overrange conditions
//------------------------------------------------------------------------------

BOOL siiCheckOutOfRangeConditions( SyncInfoType * SyncInfo )
{
WORD wHFreq;
BOOL qOutOfRangeF = TRUE;

    wHFreq = (WORD) ( ( (DWORD) SyncInfo->VFreq * SyncInfo->HTot)/1000);
    if ( SyncInfo->RefrTypeVHPol & INTERLACE_MASK ) { // Interlace Video Mode
        if ( ( wHFreq < INTERLACE_HFREQ_HIGH_LIMIT)&&(wHFreq > INTERLACE_HFREQ_LOW_LIMIT))
            if ( ( SyncInfo->VFreq < INTERLACE_VFREQ_HIGH_LIMIT)&&(SyncInfo->VFreq > INTERLACE_VFREQ_LOW_LIMIT)){
        ...
        if ( (wHFreq < PROGRESSIVE_HFREQ_HIGH_LIMIT)&&(wHFreq > PROGRESSIVE_HFREQ_LOW_LIMIT)){
...
// Function Description: this function gets Colorimetry information from video input resolution
//------------------------------------------------------------------------------
...
// Function Description: this function gets Pixel Replication Rate from entry of detected video
//                       resolution
//------------------------------------------------------------------------------
...
static BYTE siiIsInterlaceMode( BYTE VidResId ){
    ...
    return ( VModeTables[VidResId].Tag.RefrTypeVHPol & SiI_InterlaceMask );
}
//------------------------------------------------------------------------------
...
BYTE bBlankDataCh[3];
    ...
    else if ( SiI_Inf.AVI.bInputColorSpace == SiI_RX_ICP_YCbCr444 ) { // YCbCr 4:4:4
        bBlankDataCh[0] = 0x80;
        bBlankDataCh[1] = 0x10;
        bBlankDataCh[2] = 0x80;
    }
    else if (SiI_Inf.AVI.bInputColorSpace == SiI_RX_ICP_YCbCr422) {   // YCbCr 4:2:2
        bBlankDataCh[0] = 0x00;
        bBlankDataCh[1] = 0x10;
        bBlankDataCh[2] = 0x80;
    }
...
// Function Description:  This function is used to detect that resolution has stated to change
//------------------------------------------------------------------------------
BOOL siiCompareNewOldSync(SyncInfoType * OldSyncInfo, SyncInfoType * NewSyncInfo ) {
BOOL qResult = FALSE;

  if ( ( abs_diff(OldSyncInfo->HTot, NewSyncInfo->HTot) <= LINES_TOLERANCE) &&
       ( abs_diff(OldSyncInfo->ClocksPerLine, NewSyncInfo->ClocksPerLine) <= PIXELS_TOLERANCE) ){
      qResult = TRUE;
  }
...
// Function Description: Sets parameters which used for generation Composite sync for
//                       particular Interlace Modes
//------------------------------------------------------------------------------

static void ConfigInterlaceCSync( BYTE bVidResId ){

    siiWriteWordHDMIRXP0( RX_CAPHALFLINE_ADDR,InterlaceCSync[ bVidResId ].CapHalfLineCnt);
    siiWriteWordHDMIRXP0( RX_PREEQPULSCNT_ADDR,InterlaceCSync[ bVidResId ].PreEqPulseCnt);
    siiWriteWordHDMIRXP0( RX_SERRATEDCNT_ADDR,InterlaceCSync[ bVidResId ].SerratedCnt);
    siiWriteWordHDMIRXP0( RX_POSTEQPULSECNT_ADDR,InterlaceCSync[ bVidResId ].PostEqPulseCnt);

}
//------------------------------------------------------------------------------
// Function Name: siiConfigureSyncOnYForInretlaceModes()
// Function Description: this functio is used to select which table should be used
//                       for generation of Sync for Interlace Modes
//------------------------------------------------------------------------------
...
BYTE bEWCode = FALSE;
    ...
    if ( bColorimetry == SiI_RX_ITU_601 )
        siiIIC_RX_RWBitsInByteP0( RX_VID_CTRL_ADDR, RX_BIT_BT709_RGBCONV | RX_BIT_BT709_YCbCrCONV, CLR );
    else if( bColorimetry == SiI_RX_ITU_709 )
        siiIIC_RX_RWBitsInByteP0( RX_VID_CTRL_ADDR, RX_BIT_BT709_RGBCONV | RX_BIT_BT709_YCbCrCONV, SET );
    else {
    ...
    if ( bVidResIndex < PC_BASE ) {
        ...
        if ( VModeTables[ bVidResIndex ].ModeId.Mode_C2 ){
            ...
            if ( SiI_Inf.AVI.bAVI_State == SiI_RX_GotAVI ) {
                bVIC = siiGetVIC(); // get Video Id Code form Info Frame Packet
...
static WORD ConvNLinesInFieldIntoNLinesInFrame( WORD bLines ){
    ...
    if ( SiI_Inf.Sync.RefrTypeVHPol & INTERLACE_MASK ){
        if ( bLines & 0x0001 ) // if odd
            ...
            bLines += ( bLines + 1 );         // calculate total number of lines in frame
    }
    return bLines;
}
//------------------------------------------------------------------------------
// Function Name: ConvertClocksToPixels
// Function Description: For pixel replicated modes it's need to convert clocks to pixels
//                       for not replicated modes clocks equal to pixels
//------------------------------------------------------------------------------
...
BYTE abData[4];
    ...
    pbVideoResInfo[ 1 ] = ( SiI_Inf.AVI.bPixRepl  << 4 ); //added for new API
...
//    pbVideoResInfo[ 1 ] = SiI_Inf.Sync.RefrTypeVHPol;  //YMA changed to support API rev2
    ...
    ConvertClocksToPixels ( &wAux );                     // for pixel replicated resolutions take out replicated pixels
...
//                       Pix Replication Divider, Output Divider, Color Converter Type (BT601 vs. BT701)
...
BYTE bECode = FALSE;
    ...
    SetVideoBlankLevel( SiI_Inf.bVResId );  // PC modes and CE have different blank levels
    ...
    if ( !bECode ) {
        siiSetVidResDependentVidPath( SiI_Inf.AVI.bPixRepl, bVideoPathSelect );     // TODO name

        if( ( SiI_Ctrl.VideoF.bOutputSyncSelect == SiI_RX_AVC_SOGY ) && siiIsInterlaceMode( bVidResId ) ) {
            if ( ( SiI_Ctrl.bDevId ==  RX_SiI9021 ) || ( SiI_Ctrl.bDevId ==  RX_SiI9031 ) )//YMA changed by vk for in new API
...
// Function Description: This function prints out found video mode information, used for debugging purpose
//------------------------------------------------------------------------------
#ifdef SII_DUMP_UART
