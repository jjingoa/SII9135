...
// Function Description: This function reads RX interrupts and used for debugging purpose only
//
// Accepts: none
// Returns: none
// Globals: none
...
BYTE i, abIntrData[7];
    ...
    siiReadBlockHDMIRXP0( RX_HDMI_INT_GR1_ADDR - 1, 5, abIntrData );  // including main status reg
...
static void SetNoAVIIntrrupts( BOOL qOn ){
    if ( qOn ) {
        siiIIC_RX_RWBitsInByteP0( RX_HDMI_INT_ST4_MASK_ADDR, RX_BIT_MASK_NO_AVI_INTR, SET );
        SiI_Ctrl.bIgnoreIntr |= qcIgnoreNoAVI;
    }
    else {
        siiIIC_RX_RWBitsInByteP0( RX_HDMI_INT_ST4_MASK_ADDR, RX_BIT_MASK_NO_AVI_INTR, CLR );
        SiI_Ctrl.bIgnoreIntr &= (~qcIgnoreNoAVI);
    }
}
//------------------------------------------------------------------------------
...
// Function Description: This function enables using VSync Interrupts for processing HDCP errors
//                        on field to field base or diable the mode with switching wait for isolated HDCP
//                        Failure
//------------------------------------------------------------------------------
static void EnableCheckHDCPFailureWithVSyncRate ( BOOL qOn ){
    ...
    if ( qOn ){
        ...
        SiI_Ctrl.bIgnoreIntr |= qcIgnoreHDCPFail; // don't clear HDCP Failure Int if
                                                  // this bit is set
        siiIIC_RX_RWBitsInByteP0( RX_HDMI_INT_ST2_MASK_ADDR, RX_BIT_VSYNC, SET );
        siiIIC_RX_RWBitsInByteP0( RX_HDMI_INT_ST4_MASK_ADDR, RX_BIT_HDCP_FAILED, CLR );
    }
    else {
        SiI_Ctrl.bHDCPFailFrmCnt = 0;
        SiI_Ctrl.bIgnoreIntr &= (~qcIgnoreHDCPFail);
        siiIIC_RX_RWBitsInByteP0( RX_HDMI_INT_ST2_MASK_ADDR, RX_BIT_VSYNC, CLR );
        siiIIC_RX_RWBitsInByteP0( RX_HDMI_INT_ST4_MASK_ADDR, RX_BIT_HDCP_FAILED, SET );
    }
...
#endif
                    ReqReAuthenticationRX();  // Clearing Ri will be notification for host to start
...
// Handling second group interrupts
	...
	siiReadBlockHDMIRXP0( RX_HDMI_INT_GR2_ADDR, 2, abIntrData ); //YMA NOTE R 0x7B, 0x7C

//YMA should be bug, AAC is not in abIntrData[1]
//	abIntrData[1] &= (~( RX_BIT_ACP_PACKET | RX_BIT_AAC_DONE));  // This interrupt has been processed on polling base

	abIntrData[1] &= ( ~RX_BIT_ACP_PACKET );  // This interrupt has been processed on polling base
	siiWriteBlockHDMIRXP0( RX_HDMI_INT_GR2_ADDR, 2, abIntrData);

	if (( abIntrData[0] & RX_BIT_AAC_DONE) && (!(SiI_Ctrl.bIgnoreIntr & qcIgnoreAAC)) ) {
		siiTurnAudioOffAndSetSM_AudioOff();
	...
	if ( ( abIntrData[0] & RX_BIT_HRES_CHANGE) || ( abIntrData[0] & RX_BIT_VRES_CHANGE ) ) {
    // it is caused a false interrupt, by pix replication change, before unmute we well recheck
...
//9135 only
    if (( SiI_Ctrl.bDevId == RX_SiI9135|| SiI_Ctrl.bDevId ==  RX_SiI9125 )&&( abIntrData[1] & BIT_HBRA_ON_CHANGED )){
		SiI_Ctrl.sm_bAudio = SiI_RX_AS_ReqAudio; 
...
// Handling first group interrupts
	...
	siiReadBlockHDMIRXP0( RX_HDMI_INT_GR1_ADDR, 4, abIntrData); // Copy Interrupt Events into RAM
	abIntrData[0] &= ( ~RX_BIT_CTS_CHANGED );  // This interrupt has been processed on polling base
	abIntrData[1] &= ( ~( RX_BIT_GOT_AUDIO_PKT | RX_BIT_GOT_CTS_PKT) );  // This interrupt has been processed on polling base

	if ( SiI_Ctrl.bIgnoreIntr & qcIgnoreHDCPFail ) {
		if ( !(abIntrData[1] & RX_BIT_VSYNC) ) {
			abIntrData[3] &= ( ~RX_BIT_HDCP_FAILED ); // this interrupt shouldn't be cleared because no VSync
		}
	}

	siiWriteBlockHDMIRXP0( RX_HDMI_INT_GR1_ADDR, 4, abIntrData); // Clear Interrupts

	if ( abIntrData[1] & RX_BIT_SCDT_CHANGE ) {
		EnableCheckHDCPFailureWithVSyncRate( OFF );
		siiMuteVideoAndSetSM_SyncInChange();
		if ( ( SiI_Ctrl.bDevId ==  RX_SiI9021 ) || ( SiI_Ctrl.bDevId ==  RX_SiI9031 ) ) {    // SiI9031/21 cannot auto-mute when no SCDT
			siiTurnAudioOffAndSetSM_AudioOff();
			}
        siiSetAFEClockDelay();
#ifdef SII_BUG_PHOEBE_AUTOSW_BUG
...
//		if ( abIntrData[0] & RX_BIT_HDCP_DONE ) {
		...
		if ( ( abIntrData[3] & RX_BIT_HDCP_FAILED ) || ( SiI_Ctrl.bIgnoreIntr & qcIgnoreHDCPFail ) ) {
			bError = HDCPErrorHandler ( abIntrData[3] & RX_BIT_HDCP_FAILED, abIntrData[1] & RX_BIT_VSYNC );
		}

		if ( ( abIntrData[2] & ST3_INFO_MASK) || ( abIntrData[3] & ST4_INFO_MASK) ) {
			if ( ( abIntrData[3] & RX_BIT_MASK_NO_AVI_INTR ) && ( SiI_Ctrl.bIgnoreIntr & qcIgnoreNoAVI ) ) {
...
// Function Description: checks RX global interrupt bit, used when polling RX interrupts
//------------------------------------------------------------------------------
static BOOL CheckRXInt_Bit( void ){
BOOL qResult = FALSE;

    if ( siiReadByteHDMIRXP0( RX_INTR_STATE_ADDR ) & RX_BIT_GLOBAL_INTR )
        qResult = TRUE;
    return qResult;
}
//------------------------------------------------------------------------------
// Function Name: siiRX_BitInterruptHandler
// Function Description: Checks global interrupt bit, and call RX interrupt handler if
//                       bit is One
// Accepts: none
// Returns: none
// Globals: none
