...
// Function Description: This function sets TMDS registers respecting input color color depth
// Accepts: none
// Returns: none
// Globals: none
//------------------------------------------------------------------------------
void siiResetDeepColorMode ( void ) {
    ...
    if( SiI_Ctrl.bDevId != RX_SiI9133 )               // reset video FIFO feature only in 9135 and/or after
        siiVideoFIFO_Reset();                                                                        //reset and flush    video FIFO
    siiIIC_RX_RMW_ByteP0 ( RX_TMDS_ECTRL_ADDR, RX_DC_CLK_CTRL_MASK, RX_DC_CLK_8BPP_1X );
...
BYTE bError = FALSE;
    ...
    switch ( bOutputColorDepth ) {
        case SiI_RX_CD_24BPP:
            siiIIC_RX_RMW_ByteP0 (RX_VID_MODE2_ADDR, RX_DITHER_MODE_MASK, RX_DITHER_8BITS);
            break;
        case SiI_RX_CD_30BPP:
            siiIIC_RX_RMW_ByteP0 (RX_VID_MODE2_ADDR, RX_DITHER_MODE_MASK, RX_DITHER_10BITS);
            break;
        case SiI_RX_CD_36BPP:
            siiIIC_RX_RMW_ByteP0 (RX_VID_MODE2_ADDR, RX_DITHER_MODE_MASK, RX_DITHER_12BITS);
            break;
        default:
...
BYTE bError = FALSE;
    ...
    bColorDepth = siiReadByteHDMIRXP0(RX_DC_STATUS_ADDR) & RX_DC_PIXELDEPTH_MASK;                               // Read incoming pixel depth from latest General Control Packet

        if ( bColorDepth != SiI_Inf.AVI.bInputColorDepth) { // If it differs from the current setting

        siiMuteVideo(ON);

        if(SiI_Ctrl.bDevId == RX_SiI9135|| SiI_Ctrl.bDevId ==  RX_SiI9125)                                                               //reset video FIFO feature only in 9135 and/or after
           siiVideoFIFO_Reset();                                                                        //reset and flush    video FIFO

        SiI_Inf.AVI.bInputColorDepth = bColorDepth;                                                        // Update the current setting
        ...
        switch (bColorDepth) {                                                                                  // Update the value in hardware
            ...
            case RX_DC_10BPP_VAL:
                siiIIC_RX_RMW_ByteP0( RX_TMDS_ECTRL_ADDR, RX_DC_CLK_CTRL_MASK, RX_DC_CLK_10BPP_1X );
                break;
            case RX_DC_12BPP_VAL:
                siiIIC_RX_RMW_ByteP0( RX_TMDS_ECTRL_ADDR, RX_DC_CLK_CTRL_MASK, RX_DC_CLK_12BPP_1X );
                break;
