...
// Function Description: checks video state machine, state is used when res change
//                       interrupt is ignorred
// Accepts: none
...
// Function Description:   Sets SiI_RX_VS_ReqGlobalPowerDown and when SiI_RX_VS_GlobalPowerDown
//                         will be asserted
//------------------------------------------------------------------------------
...
// Function Description:   Sets SiI_RX_VS_ReqPowerDown and when SiI_RX_VS_PowerDown
//                         will be asserted
//------------------------------------------------------------------------------
    ...
    if ( !(SiI_Inf.bGlobStatus & SiI_RX_GlobalHDMI_Detected) ) {
        ...
        if ( SiI_Inf.AVI.bInputColorSpace != SiI_RX_ICP_RGB ){
            ...
            siiSetVideoPathColorSpaceDependent( SiI_Ctrl.VideoF.bOutputVideoPath, SiI_Inf.AVI.bInputColorSpace );
                                ...
                                // a call to config. Video Path non res. dependent Vid. Path
        }
    }
...
SyncInfoType SyncInfo;
    ...
    siiGetSyncInfo( &SyncInfo );  // in case if Pix Replication was changed
    ...
    if ( siiCompareNewOldSync( &SiI_Inf.Sync, &SyncInfo ) ){
        siiMuteVideo( OFF );
        SiI_Ctrl.sm_bVideo = SiI_RX_VS_VideoOn;
//        InitTX();  // TBD: This User call should be removed - Needed for Oscar board only
    }
    else
        siiMuteVideoAndSetSM_SyncInChange();
}
