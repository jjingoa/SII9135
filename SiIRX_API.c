...
WORD wStartSysTimerTicks;
    ...
    return siiGetErrorsWarnings();   // if Result FALSE, no errors and warning messages
...
//                       This command does not change the behavior of any device's
...
// Globals: <global variables and system state changes>
//------------------------------------------------------------------------------
BYTE SiI_RX_DoTasks (  BYTE * pbRXParameters  ){  // Slot time is time between last and current DoTasks measured in ms
WORD wNumberOfTicks;
...
WORD wStartSysTimerTicks;
    ...
    if ( siiCheckIfVideoInputResolutionReady( ) )
        pbRXParameters [2] = siiGetVideoResId ( SiI_Inf.bVResId );       // is BYTE bModeId
    else {
    ...
    pbRXParameters [3] = SiI_Inf.AVI.bInputColorSpace; // is BYTE bInputColorSpace
    pbRXParameters [4] = SiI_Inf.AVI.bColorimetry//;     // is BYTE bColorimetry
                        | (SiI_Inf.AVI.bInputColorDepth << 4);     // is  upper 4 bits is Color Depth
    pbRXParameters [5] = SiI_Inf.bHDCPStatus; // is BYTE bHDCPStatus
    pbRXParameters [6] = SiI_Inf.bGlobStatus; // is BYTE bGlobStatus
...
// Function Description:  Get the current Info Frame received by the Silicon Image HDMI receiver.
//
...
// ---
// Function Description: Sets Video Output Format of SiI RX, function sets static part
//                      of Video Path, dynamic part will be complete after detection of
//                      video resolution
...
BYTE abVidFormatData[3];
    ...
    siiGetVideoFormatData( abVidFormatData );       //YMA restore the configuration, or outpout depth losted		
    ...
    siiSetVideoFormatData( abVidFormatData ); // Update Video format data to  Vid. Mode/Ctrl regs.

    SiI_Ctrl.VideoF.bOutputVideoPath = bOutputVideoPathSelect; // later will be used in Vid. Res. Dependent functions
...
WORD wStartSysTimerTicks;
    ...
    pbRXParameters[0] = SiI_Ctrl.VideoF.bOutputVideoPath; // later will be used in Vid. Res. Dependent functions
...
// Function Description: Set the Silicon image HDMI receivers audio output format.
//                       Both audio interfaces, SPDIF and I2S can be selected simultaneously.
//                       The Smooth HW Mute can also be enabled regardless of
//                       the audio interfaces combination selected.
//
//                       The BYTE bI2SMap is a reserved parameter in the API specification.
//                       It is being used to set up the I2S mapping, the API does not support
//                       configurations other than the default.
// Accepts: none
// Returns: none
// Globals: none
//------------------------------------------------------------------------------
BYTE SiI_RX_SetAudioOutputFormat ( WORD wAudioOutputSelect,
                                   WORD wI2SBusFormat,
                                    BYTE bI2SMap ,
									BYTE bDSDHBRFormat){ //YMA 2 added for API rev2
WORD wStartSysTimerTicks;
...
//YMA 2 add to save the configuration for later restore when HBR/DSD -> PCM
...
//
// Accepts: none
// Returns: pbRXParameters [0] is BYTE bAudioOutputControl, pbRXParameters [1] is WORD wI2SBusFormat
...
WORD wStartSysTimerTicks;
    ...
    if (siiIsAudioStatusReady()){
        ...
        pbRXParameters[1] =  SiI_Inf.AudioStatus.bRepresentation;         // Compressed, PCM, DSD
...
BYTE bNumberWrn;
    ...
    siiMeasureProcLastAPI_Ticks( wStartSysTimerTicks );
    return bNumberWrn;   // if Result FALSE, no errors and warning messages
...
BYTE bNumberErr;
    ...
    siiMeasureProcLastAPI_Ticks( wStartSysTimerTicks );
    return bNumberErr;   // if Result FALSE, no errors and warning messages
}
...
// Function Description:   Get information about N/CTS packets, these packets are sent
//                         by TX and used for Audio Clock Regeneration (ACR)
// Accepts: none
...
// Function Description:   Get AKSV and BKSV ( for more information see HDCP specefication)
//
// Accepts: none
// Returns: pointer on requested data
// Globals: none
