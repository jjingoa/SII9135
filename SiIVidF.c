...
// Function Description: This function reads video format data from control registers
//
...
// Function Description: This function writes video format data into control registers
//
// Accepts: pointer on Video Format Data
// Returns: pointer on Video Format Data
// Globals: none
...
InputColorSpace == SiI_RX_ICP_RGB | InputColorSpace == SiI_RX_ICP_YCbCr444 | InputColorSpace == SiI_RX_ICP_YCbCr422

                             [4A:2] VID_M [4A:1] VID_M   [4A:3]  VID_M  [49:2] VID_M2  [49:0] VID_M2
InColorSpace  OutColorSpace  UpScale Bit  DownScale Bit  RGB->YCbCr     YCbCr->RGB     RGBAout
                             422->444       444->422
...
// Function Description: Preparing Video Path's bits of Video Output Format structure for RGB Input Color Space
...
//                                       |    7   |   6    |   5        |       4         |    3           |    2      |   1    |   0      |
// pbVideoOutputFormat[0] 0x48 VID_CTRL  |InvVSync|InvHSync|CSyncOnVSync|CSyncOnHSync     |enSOG           |YCbCr2RGB  |BitMode |RGB2YCbCr |
// pbVideoOutputFormat[1] 0x49 VID_MODE2 |Res     |Res     |Res         |LE Pol           |En YCbCr2RGB Rng|EnYCbCr2RGB|En Ped. |DAC RGB   |
// pbVideoOutputFormat[2] 0x4A VID_MODE  |SyncCode|MuxYC   |Dither      |En RGB2YCbCr Rng |En RGB2YCbCr    |UpSmpl     |DownSmpl|EnImsCSync|

    pbVideoOutputFormat[2]&=( ~( RX_BIT_444to422 | RX_BIT_422to444 | RX_BIT_DITHER | RX_BIT_MUX_YC | RX_BIT_RGBToYCbCr ) );
    ...
    switch ( bVideoPath ) {
        ...
        case SiI_RX_P_YCbCr422_MUX8B:         // YCbCr 422 muxed 8 bit, used with digital output only
                                pbVideoOutputFormat[2]|= ( RX_BIT_RGBToYCbCr|RX_BIT_444to422|RX_BIT_MUX_YC|RX_BIT_DITHER );
                                break;
        case SiI_RX_P_YCbCr422_MUX10B:        // YCbCr 422 muxed 10 bit, used with digital output only
                                pbVideoOutputFormat[2]|= ( RX_BIT_RGBToYCbCr|RX_BIT_444to422|RX_BIT_MUX_YC);
                                break;
        case SiI_RX_P_YCbCr422_16B:
                                pbVideoOutputFormat[2]|= ( RX_BIT_RGBToYCbCr|RX_BIT_444to422|RX_BIT_DITHER);
                                break;
...
//    pbVideoOutputFormat[0]&=( ~RX_BIT_EXT_BIT_MODE ); //YMA changed as JAPAN customer reported
	pbVideoOutputFormat[0]|= RX_BIT_EXT_BIT_MODE ;
...
// pbVideoOutputFormat[2] 0x4A VID_MODE  |SyncCode|MuxYC   |Dither      |En RGB2YCbCr Rng |En RGB2YCbCr    |UpSmpl     |DownSmpl|EnImsCSync|
     ...
     pbVideoOutputFormat[2]&=( ~( RX_BIT_422to444 | RX_BIT_DITHER | RX_BIT_MUX_YC | RX_BIT_RGBToYCbCr) );
     pbVideoOutputFormat[1]&=( ~RX_BIT_YCbCrToRGB);
     pbVideoOutputFormat[0]&= ( ~RX_BIT_EXT_BIT_MODE);
     ...
     switch ( bVideoPath ) {
        ...
        case SiI_RX_P_YCbCr422:
                                ...
                                break;
        case SiI_RX_P_YCbCr422_MUX8B:         // YCbCr 422 muxed 8 bit, used with digital output only
                                pbVideoOutputFormat[2]|= ( RX_BIT_444to422 | RX_BIT_MUX_YC | RX_BIT_DITHER );
                                break;
        case SiI_RX_P_YCbCr422_MUX10B:        // YCbCr 422 muxed 10 bit, used with digital output only
...
// Returns: Warning Byte, pointer on Video Format Data, Video Path Select Data can be modified
...
// Returns: Warning byte, pointer on Video Format Data, sync select data can be modified
// Globals: none
//------------------------------------------------------------------------------
...
// pbVideoOutputFormat[2] 0x4A VID_MODE  |SyncCode|MuxYC   |Dither      |En RGB2YCbCr Rng |En RGB2YCbCr    |UpSmpl     |DownSmpl|EnImsCSync|
    ...
    pbVidFormatData[0] &= ( ~( RX_BIT_ENCSYNC_ON_HSYNC  | RX_BIT_ENCSYNC_ON_VSYNC  | RX_BIT_INSERT_CSYNC_ON_AOUT ) );
...
// Returns: Warning byte, pointer on Video Format Data, sync control data can be modified
// Globals: none
//------------------------------------------------------------------------------
BYTE siiPrepSyncCtrl( BYTE bSyncCtrl, BYTE * pbVidFormatData ){
BYTE bECode = FALSE;
//                                     |    7   |   6    |   5        |       4         |    3           |    2      |   1    |   0      |
// pbVidOutputFormat[0] 0x48 VID_CTRL  |InvVSync|InvHSync|CSyncOnVSync|CSyncOnHSync     |enSOG           |YCbCr2RGB  |BitMode |RGB2YCbCr |
// pbVidOutputFormat[1] 0x49 VID_MODE2 |Res     |Res     |Res         |LE Pol           |En YCbCr2RGB Rng|EnYCbCr2RGB|En Ped. |DAC RGB   |
// pbVidOutputFormat[2] 0x4A VID_MODE  |SyncCode|MuxYC   |Dither      |En RGB2YCbCr Rng |En RGB2YCbCr    |UpSmpl     |DownSmpl|EnImsCSync|
...
static void Configure2PixPerClockMode( BOOL On ){
    ...
    if ( On ){
        siiIIC_RX_RWBitsInByteP0( RX_SYS_CTRL1_ADDR , RX_BIT_2PIX_MODE, SET );
        siiIIC_RX_RWBitsInByteP1( RX_PD_SYS2_ADDR , RX_BIT_PD_QE, SET );
    }
    else {
        siiIIC_RX_RWBitsInByteP0( RX_SYS_CTRL1_ADDR , RX_BIT_2PIX_MODE, CLR );
        siiIIC_RX_RWBitsInByteP1( RX_PD_SYS2_ADDR , RX_BIT_PD_QE, CLR );
    }
...
//------------------------------------------------------------------------------
static void SetInputOutputDivider( BOOL qMode48BitOr2PixClk, BOOL qRestoreOrigPixClock, BYTE PixRepl ){
BYTE bRegVal;
    SiI_Ctrl.bShadowPixRepl = PixRepl;
    bRegVal = siiReadByteHDMIRXP0( RX_SYS_CTRL1_ADDR );

    bRegVal &= RX_MSK_CLR_IN_OUT_DIV;   // clear bits of input and output devider

    bRegVal |= ( PixRepl << 4 );        // Set bits for input divider equal Pixel Replication

    if ( (!qRestoreOrigPixClock) && qMode48BitOr2PixClk ) {
                bRegVal |= RX_OUTPUT_DIV_BY_2;
    }
    else if ( qRestoreOrigPixClock && (!qMode48BitOr2PixClk) )
        bRegVal |= ( PixRepl << 6 );   // Set bits for output divider to restore original pixel clock
    else if ( qRestoreOrigPixClock && qMode48BitOr2PixClk ){
    ...
    switch ( bVideoPath ) {
        ...
        case SiI_RX_P_YCbCr422_20B_2PixClk:   // SiI9011
            if ( SiI_Ctrl.bDevId ==  RX_SiI9011 ) {
                ...
                SetInputOutputDivider( ON /*On 48bit(LE) or 2Pix*/, qRestoreOrgPixClock, bPixRepl );
            }
            else
                siiWarningHandler( SiI_WC_ChipNoCap );
        ...
        case SiI_RX_P_YCbCr444_48B:           // SiI9021/31
                ...
                break;
        default:
                SetInputOutputDivider( OFF/*Off 48bit (LE) and 2Pix*/, qRestoreOrgPixClock, bPixRepl );
                if ( SiI_Ctrl.bDevId ==  RX_SiI9011 )
...
void siiMuteVideo( BYTE On ){
    ...
    if ( On ) {
        siiIIC_RX_RWBitsInByteP1( RX_AUDP_MUTE_ADDR, RX_BIT_VIDEO_MUTE, SET );
        SiI_Inf.bGlobStatus |= SiI_RX_Global_VMute;
    }
    else {
        siiIIC_RX_RWBitsInByteP1( RX_AUDP_MUTE_ADDR, RX_BIT_VIDEO_MUTE, CLR );
        SiI_Inf.bGlobStatus &= (~SiI_RX_Global_VMute);
    }
}
