...
//
// Accepts: pointer on AudioOutputFormatType
// Returns: none
// Globals: none
//------------------------------------------------------------------------------
...
*/
	//yma for API rev2
	//actually not need read from the registers because it may be the settings for I2S, not DSD/HBR
	//should return the settings by API set function or from EEPROM
	//otherwise, the DSD/HBR configurations will be overwritten by the I2S settings.
		...
		if(SiI_Inf.AudioStatus.bRepresentation == SiI_RX_AudioRepr_DSD){
			//YMA 2 set DSD output format as set from API
				if(SavedAudioOutputFormat.bDSDHBRFormat & SiI_RX_AOut_DSD_WS16Bit) 
					siiIIC_RX_RWBitsInByteP1(RX_I2S_CTRL1_ADDR,RX_BIT_Aout_WordSize,SET);
				if(SavedAudioOutputFormat.bDSDHBRFormat & SiI_RX_AOut_DSD_SENeg)
					siiIIC_RX_RWBitsInByteP1(RX_I2S_CTRL1_ADDR,RX_BIT_Aout_ClockEdge,SET);
		}
...
//-----------------------------------------------------------------------------
ROM const CTSLimitsType CTSLimits[4] = {
...
void siiAudioMute( BOOL qOn ){
    ...
    if ( qOn ) {
        siiIIC_RX_RWBitsInByteP1( RX_AUDP_MUTE_ADDR, RX_BIT_AUDIO_MUTE, SET );
        SiI_Inf.bGlobStatus |= SiI_RX_GlobalHDMI_AMute;
    }
    else {
        siiIIC_RX_RWBitsInByteP1( RX_AUDP_MUTE_ADDR, RX_BIT_AUDIO_MUTE, CLR );
        SiI_Inf.bGlobStatus &= (~SiI_RX_GlobalHDMI_AMute);
    }
}
...
// Function Description: for switching between Automatic and Manual Audio control
//------------------------------------------------------------------------------
static void AudioExceptionsControl(BOOL qOn){

    if ( qOn ) {
        siiIIC_RX_RWBitsInByteP0( RX_AEC_CTRL_ADDR, RX_BIT_AEC_EN, SET );
        siiIIC_RX_RWBitsInByteP0( RX_INT_MASK_ST5_ADDR,  RX_BIT_AAC_DONE, SET );
        SiI_Ctrl.bIgnoreIntr &= (~qcIgnoreAAC);
    }
    else {
        siiIIC_RX_RWBitsInByteP0( RX_AEC_CTRL_ADDR, RX_BIT_AEC_EN, CLR );
        siiIIC_RX_RWBitsInByteP0( RX_INT_MASK_ST5_ADDR,  RX_BIT_AAC_DONE, CLR );
        SiI_Ctrl.bIgnoreIntr |= qcIgnoreAAC;
...
// Function Description: This function makes initialisation of Audio Clock Regeneration (ACR)
//------------------------------------------------------------------------------
...
static void ClearFIFOOverUnderRunInterrupts( void ){
      ...
      siiWriteByteHDMIRXP0( RX_HDMI_INT_ST4_ADDR, RX_BIT_FIFO_UNDERRUN | RX_BIT_FIFO_OVERRUN );
}
//------------------------------------------------------------------------------
// Function Name: CheckFIFOOverUnderRunInterrupts
// Function Description: used to detect the interrupt, getting of these interrupt,
//                       tells that FIFO has problems
//------------------------------------------------------------------------------
static BOOL CheckFIFOOverUnderRunInterrupts( void ){
BOOL qResult = FALSE;

    if ( siiReadByteHDMIRXP0( RX_HDMI_INT_ST4_ADDR) & ( RX_BIT_FIFO_UNDERRUN | RX_BIT_FIFO_OVERRUN ) )
        qResult = TRUE;
    return  qResult;
}
...
// Function Description:  indicates presence of CTS Packets, these packets used for
...
void siiClearGotCTSAudioPacketsIterrupts ( void ) {
    siiWriteByteHDMIRXP0( RX_HDMI_INT_ST2_ADDR, ( RX_BIT_GOT_AUDIO_PKT | RX_BIT_GOT_CTS_PKT) );
}
...
BOOL qResult = FALSE;
    ...
    else { // This check goes when video resolution is not detected yet,
...
// Function Description: clears CTS dropped or reused interrupts
//                       these interrupts used to check stability of receiving CTS
//                       packets
//------------------------------------------------------------------------------
static void ClearCTS_DroppedReusedInterrupts( void ){
    siiWriteByteHDMIRXP0( RX_HDMI_INT_ST4_ADDR , RX_BIT_CTS_DROPPED | RX_BIT_CTS_REUSED );
}
//------------------------------------------------------------------------------
...
BOOL qResult = FALSE;
    ...
    if ( siiReadByteHDMIRXP0( RX_HDMI_INT_ST4_ADDR ) & ( RX_BIT_CTS_DROPPED | RX_BIT_CTS_REUSED ))
        qResult = TRUE;
    return qResult;
}
//------------------------------------------------------------------------------
// Function Name: IsAudioStable()
// Function Description:  About stable audio clock we judge how stable CTS packets are coming
...
// Function Description: Check if Audio PLL is still unlock. Function clears PLL unlock
...
// Function Description: If no FIFO under run or over run interrupts after Audio FIFO restet,
//                       then FIFO is working fine
//------------------------------------------------------------------------------
...
BYTE bFIFO_DiffPointer;
    ...
    if ( ( SiI_Ctrl.bDevId ==  RX_SiI9021 ) || ( SiI_Ctrl.bDevId ==  RX_SiI9031 ) ){
        ...
        do { // !!!!!!!!! wait when FIFO UnderRun and OverRun are gone
...
BYTE bRegVal;
    ...
    siiWriteByteHDMIRXP1( RX_I2S_CTRL2_ADDR, bRegVal );  // Enable SDX (Reg (p1)27)

    bRegVal = siiReadByteHDMIRXP1( RX_AUDIO_CTRL_ADDR );     // Enable SCK, WS, SPDIF, ignor VUCP errors (p1)2
...
// Function Description: Checks Audio Ready and makes ACR Reaset and Initilization
//
// Accepts: none
// Returns: none
// Globals: none
...
// Function Description:  saving input Audio stream info into SiI_Inf.AudioStatus
...
// get audio sample representation
    if ( siiReadByteHDMIRXP1( RX_CH_STATUS1_ADDR ) & RX_BIT_AUDIO_SAMPLE_NPCM )
    ...
    if ( (SiI_Ctrl.bDevId ==  RX_SiI9033 || SiI_Ctrl.bDevId ==  RX_SiI9133 
		|| SiI_Ctrl.bDevId ==  RX_SiI9135|| SiI_Ctrl.bDevId ==  RX_SiI9125)
         && ( siiReadByteHDMIRXP1( RX_AUDP_STAT_ADDR ) & RX_BIT_DSD_STATUS ) ) {
        SiI_Inf.AudioStatus.bRepresentation = SiI_RX_AudioRepr_DSD;
    }
...
// get Fs/Length
    if( ( SiI_Inf.AudioStatus.bRepresentation == SiI_RX_AudioRepr_PCM )
        || ( SiI_Inf.AudioStatus.bRepresentation == SiI_RX_AudioRepr_Compressed )) {
         ...
         SiI_Inf.AudioStatus.bAccuracyAndFs |= siiReadByteHDMIRXP1( RX_CH_STATUS4_ADDR );
         SiI_Inf.AudioStatus.bLength =  ( siiReadByteHDMIRXP1( RX_CH_STATUS5_ADDR ) & 0x0F );
    }



    if ( SiI_Inf.AudioStatus.bNumberChannels == SiI_RX_NumAudCh_Unknown ){ // bNumberChannels is copied from Audio InfoFrame if it's not Zero
        if ( siiReadByteHDMIRXP1( RX_AUDP_STAT_ADDR ) & RX_BIT_LAYOUT_1 )
...
void siiSetAutoFIFOReset (BOOL qOn){
    if ( ( SiI_Ctrl.bDevId == RX_SiI9125 ) || ( SiI_Ctrl.bDevId == RX_SiI9135 ) ) {
		...
		if ( qOn )
			siiIIC_RX_RWBitsInByteP0(RX_SWRST_ADDR2, RX_BIT_AUDIOFIFO_AUTO, SET);// YMA 9135 only
		else
			siiIIC_RX_RWBitsInByteP0(RX_SWRST_ADDR2, RX_BIT_AUDIOFIFO_AUTO, CLR);
	}
}
...
//YMA removed: it is done in ISR, should not be called again here since the intr is cleared in ISR already
//siiChangeAudioStreamHandler();
    ...
    if (!CheckCTSChanged()){
        if ( CheckPLLUnLockAndReCheckAudio() ){
            SetAudioClock();
            PrepareAACOn();		// MJ: Moved outside if to fix I2S DSD Audio FIFO Bug 3056
            ...
            if ( !SiI_Inf.bAudioErr ) {
                ...
                AudioExceptionsControl(ON);
                 ...
                 WakeUpAudioDAC();
//		 	     halSetAudioDACMode(SiI_Inf.AudioStatus.bRepresentation);
                qResult = TRUE;
            }
            else {
                ClearAudioClock();
...
void DisableAudioOutputs( void ){
    ...
    siiWriteByteHDMIRXP1( RX_AUDIO_CTRL_ADDR, 0x18);     // Disable SCK, SPDIF, Enable I2S/DSD
