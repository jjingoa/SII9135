...
BYTE bRegVal;
        ...
        siiWriteByteHDMIRXP1( RX_FREQ_SVAL_ADDR, bRegVal | (  bDividerIndex << 6 ) | (  bDividerIndex << 4 ) );
...
BYTE bMCLK;
	...
	if(qON)
	{
        bMCLK = siiReadByteHDMIRXP1( RX_FREQ_SVAL_ADDR ) & 0xF0;  //reg 102
		bRegVal = siiReadByteHDMIRXP1( RX_PCLK_FS_ADDR ) & 0x0F;      //reg 117
		switch (bRegVal)
		{
			case _192Fs://192K
				bRegVal = _48Fs;//48K
...
*/
	    ...
	    siiIIC_RX_RWBitsInByteP1( RX_ACR_CTRL1_ADDR, RX_BIT_ACR_FSSEL, SET );


	}
	else
	    siiIIC_RX_RWBitsInByteP1( RX_ACR_CTRL1_ADDR, RX_BIT_ACR_FSSEL, CLR);
...
void siiRX_PowerDown (BYTE bPowerDown) {
    ...
    if ( bPowerDown == ON ) {
        ...
        if ( bSeparateAFE == PRESENT )
            siiIIC_RX_RWBitsInByteU0( RX_SYS_CTRL1_ADDR, RX_BIT_PD_ALL, CLR );
        }
    else {
        siiIIC_RX_RWBitsInByteP0( RX_SYS_CTRL1_ADDR, RX_BIT_PD_ALL, SET );
        if ( bSeparateAFE == PRESENT )
            siiIIC_RX_RWBitsInByteU0( RX_SYS_CTRL1_ADDR, RX_BIT_PD_ALL, SET );
        }
    }
...
void siiRX_DisableTMDSCores( void ){
    ...
    if ( bSeparateAFE == PRESENT )
        siiIIC_RX_RWBitsInByteU0( RX_SYS_SW_SWTCHC_ADDR, RX_BIT_RX0_EN, CLR );
}
...
void sii_SetVideoOutputPowerDown( BYTE bVideoOutputPowerDown ){
    ...
    switch ( bVideoOutputPowerDown ) {
        ...
        case SiI_RX_VidOutPD_NoPD:
                            siiIIC_RX_RWBitsInByteP1( RX_PD_SYS_ADDR, RX_BIT_NPD_VIDEO_DAC, SET );
                            siiIIC_RX_RWBitsInByteP1( RX_PD_SYS2_ADDR, RX_BIT_NPD_DIGITAL_OUTPUTS, SET );
                            break;

        case SiI_RX_VidOutPD_Analog:
                            siiIIC_RX_RWBitsInByteP1( RX_PD_SYS_ADDR, RX_BIT_NPD_VIDEO_DAC, CLR );
                            siiIIC_RX_RWBitsInByteP1( RX_PD_SYS2_ADDR, RX_BIT_NPD_DIGITAL_OUTPUTS, SET );
                            break;

        case SiI_RX_VidOutPD_AnalogAndDigital:
                            siiIIC_RX_RWBitsInByteP1( RX_PD_SYS_ADDR, RX_BIT_NPD_VIDEO_DAC, CLR );
                            siiIIC_RX_RWBitsInByteP1( RX_PD_SYS2_ADDR, RX_BIT_NPD_DIGITAL_OUTPUTS, CLR );
                            break;
...
BOOL qResult = FALSE;
    ...
    if ( siiReadByteHDMIRXP0( RX_STATE_ADDR ) &  RX_BIT_CABLE_HPD ) //YMA NOTE /R0x06/bit3/ 5V  ower detect 
        qResult = TRUE;
    return qResult;
}
//------------------------------------------------------------------------------
...
// Function Description: Initialization of Audio Exception registers which used for
//                       auto-muting
//------------------------------------------------------------------------------
...
// Function Description: Initilize RX interrupts (used with polling RX interrupt pin)
//------------------------------------------------------------------------------
...
void InitSoftwareResetReg(void){
    ...
    siiWriteByteHDMIRXP0( RX_SWRST_ADDR, RX_BIT_SW_AUTO ); // when no SCDT software reset is applied
#endif
    ...
    if ( bSeparateAFE == PRESENT )
    {
        siiWriteByteAFEU0( RX_SWRST_ADDR, RX_BIT_SW_AUTO ); // when no SCDT software reset is applied

        siiIIC_RX_RWBitsInByteP0( RX_SWRST_ADDR, 0x08 , SET );  // IP requires HDCP reset at start
...
BYTE bRegVal;
//YMA set the terminaion to 3k ohm first and then after HPD, change it back to normal
	bRegVal = siiReadByteHDMIRXP0 (RX_TMDS_TERMCTRL_ADDR);
	if(qOn)
		siiWriteByteHDMIRXP0 (RX_TMDS_TERMCTRL_ADDR, bRegVal&0xFC); //YMA clear bit 0,1 for ch0
	
	else
		siiWriteByteHDMIRXP0 (RX_TMDS_TERMCTRL_ADDR, bRegVal|0x03); //YMA set bit 0,1 for ch0
...
//YMA set the terminaion to 3k ohm first and then after HPD, change it back to normal
	bRegVal = siiReadByteHDMIRXP0 (RX_TMDS_TERMCTRL_ADDR);
	if(qOn)
		siiWriteByteHDMIRXP0 (RX_TMDS_TERMCTRL_ADDR, bRegVal&0x9F); //YMA clear bit 5,6 for ch2
	
	else
		siiWriteByteHDMIRXP0 (RX_TMDS_TERMCTRL_ADDR, bRegVal|0x60); //YMA set bit 5,6 for ch2
...
*/
    ...
    if ( siiWaitForAckHDMIRX( wTimeOut )  ){
        ...
        if (  qFPGA && (!qUrsula) )
            siiWriteByteHDMIRXP1( RX_ACR_CTRL3_ADDR, RX_CTS_THRESHOLD | RX_BIT_MCLK_LOOPBACK | 0x01);
        else
            siiWriteByteHDMIRXP1( RX_ACR_CTRL3_ADDR, RX_CTS_THRESHOLD | RX_BIT_MCLK_LOOPBACK );
        ...
        siiWriteByteHDMIRXP1( RX_LK_THRESHOLD_ADDR + 2, 0x00 );   // siiWrite threshold for PLL unlock interrupt

        if ( pbInit[0] & SiI_RX_InvertOutputVidClock ){       // set Video bus width and Video data edge
            siiWriteByteHDMIRXP0( RX_SYS_CTRL1_ADDR, RX_BIT_VID_BUS_24BIT | RX_BIT_INVERT_OUTPUT_VID_CLK );
        }
        else {
            siiWriteByteHDMIRXP0( RX_SYS_CTRL1_ADDR, RX_BIT_VID_BUS_24BIT );
        }
...
#endif
        siiWriteByteHDMIRXP1( RX_I2S_CTRL1_ADDR, 0x40 );      // SCL edge, if need customize SPDIF parameters
        siiWriteByteHDMIRXP1( RX_HDMI_CRIT1_ADDR, 0x06 );
        siiWriteByteHDMIRXP1( RX_HDMI_CRIT2_ADDR, 0x0C );

        siiWriteByteHDMIRXP0( RX_INT_CNTRL_ADDR, 0x06 );     // Set Interrupt polarity (Neg.)
        siiWriteByteHDMIRXP1( RX_AUDP_MUTE_ADDR, 0x03 );     // AudioVideo Mute ON

        siiWriteByteHDMIRXP0( RX_APLL_POLE_ADDR, 0x88 );   // Audio PLL setting
        siiWriteByteHDMIRXP0( RX_APLL_CLIP_ADDR, 0x16 );   // Audio PLL setting, improved PLL lock time

//YMA CP9135/25 reference board has 5ohm external resistor, so the TMDS termination control should set to 45 ohm  50 ohm
// instead of the default value 50 ohm
// 9125/35 has only the TQFP package, so this applies to all the devices have 9125/35 device ID

//YMA removed. set the terminaion to 3k ohm first and then after HPD, change it back to normal
//		if ( SiI_Ctrl.bDevId == RX_SiI9135 || SiI_Ctrl.bDevId ==  RX_SiI9125)
//		siiWriteByteHDMIRXP0 (RX_TMDS_TERMCTRL_ADDR, 0x0c); //45 ohm

        if ( ( SiI_Ctrl.bDevId == RX_SiI9133 )||( SiI_Ctrl.bDevId == RX_SiI9135 || SiI_Ctrl.bDevId ==  RX_SiI9125) 
			|| ( SiI_Ctrl.bDevId == RX_SiIIP11 ) ) {
            siiWriteByteHDMIRXP0( RX_TMDS_ECTRL_ADDR, 0x00 );  // set equlizer register value, optimized for different length of
        }
       else {
            siiWriteByteHDMIRXP0( RX_TMDS_ECTRL_ADDR, 0xC3 );  // set equlizer register value, optimized for different length of
        }
        siiWriteByteHDMIRXP1( RX_MPEG_DEC_ADDR, ISRC1_Type ); //  no need to process MPEG as data is screwed there

        siiWriteByteHDMIRXP1( RX_MUTE_DIV_ADDR, 0x02 ); // ramp slope for Soft Audio Mute
        ...
        if ( ( SiI_Ctrl.bVidInChannel == SiI_RX_VInCh1 ) || ( SiI_Ctrl.bDevId == RX_SiI9011 )  )
            siiWriteByteHDMIRXP0( RX_SYS_SW_SWTCHC_ADDR, bRegVal | 0x11 );
        else
            siiWriteByteHDMIRXP0( RX_SYS_SW_SWTCHC_ADDR, bRegVal | 0x22 );
