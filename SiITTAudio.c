...
// Function Description:  This function checks state machine, if state SiI_RX_AS_HoldAudioMute
//                        then return true
//
// Accepts: none
// Returns: BOOL
...
// Function Description:  assign this state and time out in order to sync Audio with Video
//
// Accepts: none
// Returns: none
...
// Function Description:  asserts  SiI_RX_AS_ReqAudio, this state starts check Audio conditions
//                        exception asserted SiI_RX_AS_HoldAudioMute state
//
//
// Accepts: none
// Returns: none
...
void siiSetSM_ReqAudio( void ){
    if ( SiI_Ctrl.sm_bAudio != SiI_RX_AS_HoldAudioMute ) { // Audio mute which is asserted by API command can be
...
//                        only by this function iI_RX_AS_HoldAudioMute can be deasserted
//------------------------------------------------------------------------------
...
static void SetSM_AudioOff( void ){
    if ( SiI_Ctrl.sm_bAudio != SiI_RX_AS_HoldAudioMute ) { // Audio mute which is asserted by API command can be
        ...
        SiI_Ctrl.wAudioTimeOut = 200;  // # ms being quiet, (not to attepmt bring up Audio)
    }
}
//------------------------------------------------------------------------------
// Function Name: siiTurnAudioOffAndSetSM_AudioOff
// Function Description: This function calls turning Audio Off, and changes state of the state machine
//
// Accepts: none
// Returns: none
// Globals: none
...
// Function Description:  This function emolliates waiting for Soft Muting, it should 
// be used to wait when audio soft mute is ramping down  
// Accepts: none											   
// Returns: none
//------------------------------------------------------------------------------
...
// Function Description: Call clear CTS change interrupt, change state machine for waiting PLL Lock
//------------------------------------------------------------------------------
...
static void PrepareTurningAudioOn( void ){
    ...
    if ( siiPrepareTurningAudioOn() ){
        ...
        SiI_Ctrl.wAudioTimeOut = SiI_RX_DelayAudio_WaitDACWakingUp; // Set wait # ms, when Audio DAC will be ready
    }
    else {
        SiI_Ctrl.sm_bAudio = SiI_RX_AS_AudioOff;
        SiI_Ctrl.wAudioTimeOut = 200; // # ms being quiet, (not to attepmt bring up Audio)
...
// Function Description: call teurning Audio On and change state machine to Audio On
//------------------------------------------------------------------------------
static void TurningAudioOn_SetSM_WaitStatusLock( void ){
    siiTurningAudio( ON );
    SiI_Ctrl.sm_bAudio = SiI_RX_AS_AudioOnWaitLockStatus;
    SiI_Ctrl.wAudioTimeOut = 700; // # ms being quiet, (not to attepmt bring up Audio)
}
//------------------------------------------------------------------------------
// Function Name:
