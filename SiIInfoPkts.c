...
BOOL qResult = FALSE;
    ...
    if ( (bInfoFrameLenth >= IF_MIN_LENGTH) && (bInfoFrameLenth <= IF_MAX_LENGTH))
        qResult = TRUE;
    return qResult;
...
// Function Description: After reading Packet through API interface, it's need to
//                       clear that Event of packet. Otherwise top layer would read it again
// Accepts: none
// Returns: none
// Globals: none
//------------------------------------------------------------------------------
...
// Function Description: gets Info Frames or Packets, if Info Frame check for its length
//                       and check sum
//------------------------------------------------------------------------------
...
BOOL qInfoFrame = TRUE;
    ...
    if ( qInfoFrame ) {
        if ( CheckForInfoFrameLength( pbInfoData[ IF_LENGTH_INDEX ] ) ){
            if ( !Check_CheckSum( pbInfoData[ IF_LENGTH_INDEX ] + IF_HEADER_LENGTH, pbInfoData ) )
    ...
    return bECode;
// TODO think how differentiate from another errors ( because getting these errors is a source fault)
}
//------------------------------------------------------------------------------
// Function Name: GetInfoPacketAddress
// Function Description:  returns address in SiI RX where Packet data were placed
//------------------------------------------------------------------------------
...
BYTE bAddr;
    ...
    switch ( bType ){
        ...
        case AVI_Type:      bAddr = RX_AVI_IF_ADDR;
            break;
        case SPD_Type:      bAddr = RX_SPD_IF_ADDR;
            break;
        case Audio_Type:    bAddr = RX_AUD_IF_ADDR;
            break;
        case ISRC1_Type:    bAddr = RX_MPEG_IF_ADDR;
            break;
        case ISRC2_Type:    bAddr = RX_UNKNOWN_IF_ADDR;
            break;
        case ACP_Type:      bAddr = RX_ACP_IP_ADDR;
            break;
        default:   bAddr = 0;
    }
    return bAddr;
...
// Function Description: write default AVI information in SiI_Inf global strucute
//------------------------------------------------------------------------------
    ...
    if ( bInfoFramesIntrrupts &  AVI_Mask ){
        ...
        if( InfoFrmPkt.bType ){
            ...
            if ( !bECode ){
                ...
                siiMuteVideoAndSetSM_SyncInChange();    // Request Program Video path
                SiI_Inf.bNewInfoPkts |= SiI_RX_NewInfo_AVI; // Set Sys Information Flag
#ifdef SII_DUMP_UART
                printf ("\nNew AVI\n");
#endif
            }
...
InfoFrPktType InfoFrmPkt;
    ...
    if ( InfoFrmPkt.bType ){
        bECode = GetInfoFramesOrPakets( InfoFrmPkt, abInfoData );
        if ( !bECode ){
            SiI_Inf.bNewInfoPkts |= SiI_RX_NewInfo_AUD;  // Set Sys Information Flag
...
void siiProcessInfoFrameIntrrupts( BYTE bInfoFramesIntrrupts ){
    ...
    if ( ( bInfoFramesIntrrupts &  NoAVI_Mask )||( bInfoFramesIntrrupts &  AVI_Mask) ) { // only AVI in AVI IF Buffere
        ProcessAVIorNoAVI( bInfoFramesIntrrupts );
    }
    if (  bInfoFramesIntrrupts & Audio_Mask ){               // only Audio in AVI IF Buffere
