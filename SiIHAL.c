//------------------------------------------------------------------------------
// Module Name:  SiIHAL
// Module Description: MCU (CPU) Hardware dependent functions have been placed here
//
...
void halDelayMS ( BYTE MS ){
    ...
    wScaleMS = (WORD)( (DWORD) ( MS * 1000 ) / SII_SYS_TICK_TIME ); // the expression can be optimized
                                                          // for simple TIME_TICK values
    while ( wScaleMS );   // Value is global and decremented by hardware timer
...
WORD wTicks;
    ...
    } while ( wTicks != wTickCounter );  // reading of ticks should be atomic
...
// Function Description: Timer 0 interrupts,  used to generate 8ms events used for
